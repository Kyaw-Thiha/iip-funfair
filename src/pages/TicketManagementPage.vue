...
export default defineComponent({
  ...
  setup() {
    ...
    const { filterByProperties, getById, update } = useApi();

    const ticketStart = 'IIPFunFair-';
    const ticketID = ref(ticketStart); //Reference No. of ticket
    const ticketUUID = ref(''); //Actual id of ticket
    ...
    const checksTicketID = async () => {
      //Checks if ticket exists on server and if it is still available
      const fetchedTickets = await filterByProperties('ticket', {
        reference_no: ticketID.value,
      });
      ...
      if (fetchedTickets.length == 0) {
        ...
        const fetchedTicket = fetchedTickets[0];

        ticketStatus.value = fetchedTicket.status;
        ticketUUID.value = fetchedTicket.id;

        if (ticketStatus.value == 'used') {
          $q.notify({
          ...
          //Fetch ticket user
          const user = await getById('users', fetchedTicket.user);

          ticketStatus.value = fetchedTicket.status;
          ticketUserName.value = user.name;
          ticketUserClass.value = user.class;
    ...
    const onQRCodeDecode = (decodedString) => {
    ...
    const changeStatus = async () => {
      if (ticketStatus.value == 'paid') {
        ...
        await update('ticket', { id: ticketUUID.value, status: 'used' });
      } else {
        //Patch status as paid
        await update('ticket', { id: ticketUUID.value, status: 'paid' });
      }

      //Notifying the user about success
      $q.notify({
        message: `The ticket is successfully ${
          ticketStatus.value == 'paid' ? 'used' : 'paid'
        }`,
        caption: 'Ask the customer to refresh the page to see changes',
        color: 'primary',
      });
